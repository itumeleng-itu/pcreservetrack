-- Fix conflicting status constraints and migrate computers
-- Remove the old constraint that doesn't include all status values
...
BEGIN
    ...
    -- Set some computers as reserved for testing (only if we have registered users)
    UPDATE public.computers 
    SET status = 'reserved',
        reserved_by = COALESCE((SELECT id::TEXT FROM public.registered WHERE role = 'student' LIMIT 1), 'test-user'),
