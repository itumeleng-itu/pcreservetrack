-- Fix the SECURITY DEFINER view issue identified by the linter
...
-- Add a test user to ensure authentication works
-- First check if the user already exists in registered table
DO $$
BEGIN
  -- Insert test user if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM registered WHERE staff_num = 'test123') THEN
