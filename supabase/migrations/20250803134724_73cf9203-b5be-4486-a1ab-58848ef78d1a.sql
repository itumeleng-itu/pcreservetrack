...
-- Add a proper test user to ensure authentication works
-- First check if the user already exists in registered table
DO $$
BEGIN
  -- Insert test user with proper staff number format if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM registered WHERE staff_num = 'test123456') THEN
    INSERT INTO registered (id, name, email, role, staff_num, is_deleted)
    VALUES (
      gen_random_uuid(),
      'Test User',
      'test@example.com',
